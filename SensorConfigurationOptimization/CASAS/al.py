...
def read_data(datafile):
    ...
    for line in datafile:
        ...
        # print('number of features in read data function is: ', cf.num_features)
...
def Test(cf_input, files, sensors):
    results = []
    
    
    cf.activitynames = cf_input.activitynames
    cf.current_seconds_of_day = cf_input.current_seconds_of_day
    cf.current_timestamp = cf_input.current_timestamp
    cf.day_of_week = cf_input.day_of_week
    cf.dominant = cf_input.dominant
    cf.sensornames = cf_input.sensornames

    cf.sensortimes = cf_input.sensortimes
    cf.data = cf_input.data
    cf.dstype = cf_input.dstype
    cf.labels = cf_input.labels
    cf.numwin = cf_input.numwin
    cf.wincnt = cf_input.wincnt
    cf.data_filename = cf_input.data_filename
    cf.filter_other = cf_input.filter_other
    cf.ignore_other = cf_input.ignore_other
    cf.cluster_other = cf_input.cluster_other
    cf.no_overlap = cf_input.no_overlap
    cf.confusion_matrix = cf_input.confusion_matrix 
    cf.mode = cf_input.mode  # TRAIN, TEST, CV, PARTITION, ANNOTATE, WRITE, LOO
    cf.model_path = cf_input.model_path
    cf.model_name = cf_input.model_name
    cf.num_activities = cf_input.num_activities
    cf.num_clusters = cf_input.num_clusters
    cf.num_sensors = cf_input.num_sensors
    cf.num_set_features = cf_input.num_set_features
    cf.features = cf_input.features
    cf.num_features = cf_input.num_features
    cf.seconds_in_a_day = cf_input.seconds_in_a_day
    cf.max_window = cf_input.max_window
    ...
    xtest = np.empty((0, k), dtype=float)
    ...
    for index1, datafilename in enumerate(files):
        try:
            ...
            xtest = np.append(xtest, cf.data, axis=0)
            ytest = np.append(ytest, cf.labels)
